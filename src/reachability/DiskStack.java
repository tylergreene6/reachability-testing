...
public class DiskStack implements Buffer {
	...
	if (subdir.exists()) {
		...
		if (!restart) {
		    ...
		    bot = min;
		    top = max;
		    System.out.println("Restart point: Bottom " + bot + " Top " + top);
		}
		else {
	...
	try {
	    String fname = FNAME_PREFIX + top + FNAME_SUFFIX;
	    top --;
	...
	try {
	    top ++;
	    String fname = FNAME_PREFIX + top + ".dat";
